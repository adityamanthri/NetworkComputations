...
public class MathSolver {
    ...
    public static double frobNorm(double[][] m){
        double sum = 0.0;
    ...
    public static double[][] matMul(double[][] m1, double[][] m2) throws Exception{
    ...
    public static double[][] scaleMat(double sc, double[][] m){
        double[][] res = m.clone();
        for(int i = 0; i < res.length; ++i){
    ...
    public static double[][] metricMDS(double[][] distMatrix) throws Exception {
        int n = distMatrix.length;

        // Create centering matrix
        double[][] H = new double[n][n];
        for (int i = 0; i < n; i++) {
            Arrays.fill(H[i], -1.0 / n);
        }

        // Compute double centering matrix

        double[][] B = distMatrix.clone();
        B = scaleMat(-0.5, B);
        B = matMul(B, H);
        B = matMul(B, H);
        B = matMul(B, distMatrix);

        // Perform eigenvalue decomposition on B
        EigenvalueDecomposition eig = new EigenvalueDecomposition(B);
        double[][] eigenvalues = eig.getD();
        double[][] eigenvectors = eig.getV();

        // Select the k largest eigenvalues and their corresponding eigenvectors
        double[][] eigenvaluesK = new double[2][2];
        double[][] eigenvectorsK = new double[n][2];
        for (int i = 0; i < 2; i++) {
            eigenvaluesK[i][i] = Math.sqrt(Math.abs(eigenvalues[n - 1 - i][n - 1 - i]));
            for (int j = 0; j < n; j++) {
                eigenvectorsK[j][i] = eigenvectors[j][n - 1 - i];
            }
        }
